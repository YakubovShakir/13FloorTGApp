...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      try {
        await Promise.all([preloadImages(), initializeProcess(), fetchNekoState()]);
        if (mountedRef.current) {
  ...
  useEffect(() => {
    ...
    const updateTimer = (displayTime) => {
      ...
      const remainingSeconds = Math.max(0, totalSeconds - elapsedSeconds);
      const formattedTime = formatTime(Math.floor(remainingSeconds / 60), remainingSeconds % 60);
      ...
      if (remainingSeconds === 0 || !canContinue(state.currentProcess?.type)) {
        ...
        setState((prev) => {
          if (prev.currentProcess?.id === processId) {
            return {
              ...prev,
              currentProcess: { ...prev.currentProcess, totalSecondsRemaining: remainingSeconds, formattedTime, totalSeconds },
            };
          }
          return prev;
        });
    ...
    const initializeTimer = async () => {
      try {
        ...
        timerInterval = setInterval(() => updateTimer(moment().tz("Europe/Moscow")), 1000);
      }
    };
              ...
              {userShelf.neko?.shelf_link && (
                <motion.div className="shelf-neko-container" style={{ position: "relative" }}>
                  <motion.img
                    ...
                    alt="neko"
                    style={{ filter: state.nekoState.canClick ? "none" : "grayscale(100%)" }}
                    transition={{ duration: 0.5, ease: "easeOut" }}
                  />
